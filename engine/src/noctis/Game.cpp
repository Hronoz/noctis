...
void Game::createImageViews()
{
    ...
    for (size_t i = 0; i < context.swapchainImages.size(); i++) {
        ...
        VK_CHECK(vkCreateImageView(context.device, &createInfo, nullptr, &context.swapchainImageViews[i]));
    }
}
...
void Game::createTextureImage()
{
    ...
    stbi_uc *pixels =
      stbi_load("engine/src/noctis/textures/texture.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
